...
"""
    N-body simulation.
    Change made: Using alternatives to membership testing of lists
    Time taken: 63.46
    R: 1.20
"""
...
def advance(bodies_keys, bodies_tbl, dt):
    '''
    ...
    for body1 in bodies_keys:
        for body2 in bodies_keys:
            if (body1 != body2) and not (seenit[body2]):
...
def report_energy(bodies_keys, bodies_tbl, e=0.0):
    '''
...
def nbody(loops, reference, iterations):
    ...
    for _ in range(loops):
        report_energy(bodies_keys, bodies_tbl)
        for _ in range(iterations):
            advance(bodies_keys, bodies_tbl, 0.01)
        print(report_energy(bodies_keys, bodies_tbl))
